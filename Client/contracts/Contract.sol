...
contract Contract {
    ...
    function getMshops() public view returns (address) {}

    function getDoctors() public view returns (address) {}

    function addMshop(address _newdr) public {
        require(admin.has(msg.sender), "Only For Admin");
        mshop.add(_newdr);
    }

    function delMshop(address shopID) public {
        require(admin.has(msg.sender), "Only For Admin");
        mshop.remove(shopID);
    }
    ...
    function isMshop(address id) public view returns (string memory) {
        require(mshop.has(id), "Only for Medical Shop");
        return "1";
    }
    ...
    */
    ...
    modifier onlyMshop() {
        require(mshop.has(msg.sender) == true, "Only Admin Can Do That");
        _;
    }
}

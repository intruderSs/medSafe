...
from api.models import Appointment, Doctor, Patient, Mshop
...
class MshopView(APIView):
    def post(self, request):
        serializer = MshopSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(
                {"status": "success", "data": serializer.data},
    ...
    def get(self, request, id=None):
        if id:
            mshop = Mshop.objects.filter(shopID=id)
            serializer = MshopSerializer(mshop, many=True)
            return Response(
                {"status": "success", "data": serializer.data},
                status=status.HTTP_200_OK,
            )
        mshops = Mshop.objects.all()
        serializer = MshopSerializer(mshops, many=True)
        return Response(
            {"status": "success", "data": serializer.data}, status.HTTP_200_OK
        )
...
class AppointmentView(APIView):
    ...
    def put(self, request, id):
        ...
        appointment.save()
        return Response(
            {"status": "success", "data": appointment.status}, status.HTTP_200_OK
        )
...
def getCount(self):
    ...
    return Response(
        {"status": "success", "docCount": doctorCount, "patCount": patientCount},
        status.HTTP_200_OK,
    )
